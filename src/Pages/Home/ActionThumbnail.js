...
function ActionThumbnail({title, text}) {
  return (
    ...
    <h2>{title}</h2>
    <div style={{width: '80px', height: '5px', backgroundColor: '#E3DDDD'}}></div>
    <p>{text}</p>
    {title === "MAKE YOUR LOGO" && <Link to="merchandise" id="LearnMoreButton">MERCH</Link>}
    {title === "WHY POWER SOCCER?" && <Link to="about" id="LearnMoreButton">ABOUT</Link>}
    {title === "FIND YOUR TEAM" && <Link to="get-involved" id="LearnMoreButton">GET INVOLVED</Link>}
    </div>
  )
